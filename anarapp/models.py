...
class Yacimiento(models.Model):
    ...
    numeroPlano = models.IntegerField('7. Numero de plano',blank=True) 
    ...
    flora = models.CharField('Flora', max_length = 1500)
    fauna = models.CharField('Fauna', max_length = 1500)
    ...
    observacionesExposicion = models.CharField('Observaciones', max_length = 150, blank=True)

    tipoExposicion = models.IntegerField('Exposicion', choices = OPCIONES_EXPOSICION)

    # Constitucion del yacimiento 

    numPiedrasYacimientoOriginal = models.IntegerField('Numero de piedras en el yacimiento original',blank=True,null=True)
    numPiedrasYacimientoGrabadas = models.IntegerField('Numero de piedras grabadas.',blank=True,null=True)
    numPiedrasYacimientoPintadas = models.IntegerField('Numero de piedras pintadas.',blank=True,null=True)
    numPiedrasYacimientoColocadas = models.IntegerField('Numero de piedras colocadas.',blank=True,null=True)

    otrosConstitucionYacimiento = models.CharField('Otras características de la constitucion yacimiento', max_length = 150)

    otrasNotas = models.CharField('2.6.10 Notas', max_length = 150,blank=True)

    estadoConservacion = models.ManyToManyField(EstadoConservacion,blank=True)
    destruccionPotencialSitio = models.ManyToManyField(DestruccionPotencialSitio,blank=True)    

    observacionesDestruccion = models.CharField('Observaciones sobre intensidad de destruccion del sitio y otros procesos no descritos.', max_length = 150,blank=True)
